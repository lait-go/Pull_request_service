...
type PullRequest struct {
	...
	AssignedReviewers []string          `json:"assigned_reviewers"` //Назначенные ревьюверы
	AuthorId          string            `json:"author_id"`          //ID автора PR
	CreatedAt         *time.Time        `json:"createdAt"`          //Время создания PR
	MergedAt          *time.Time        `json:"mergedAt"`           //Время одобрения PR
	PullRequestId     string            `json:"pull_request_id"`    //ID PR
	PullRequestName   string            `json:"pull_request_name"`  //Имя PR
	Status            PullRequestStatus `json:"status"`             //Статус PR (все виды статусов находятся выше в виде constant)
}
